...
\subsection{DBSCAN}
DBSCAN, or \textit{Density-Based Spatial Clustering of Applications with Noise}, is a clustering algorithm. It can be used for clustering data points based on density, by grouping together areas with many samples. This makes it especially useful for performing clustering under noisy conditions.\\
...
\subsection{Evaluation}
It is difficult to evaluate the validation of a clustering algorithm. A good clustering algorithm will produce high quality clusters in which the intra-class (that is, intra-cluster) similarity is high, and the inter-class similarity is low.\\
...
% valori coefficiente e wordcloud dei cluster
